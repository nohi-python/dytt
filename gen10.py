...
env.user = 'root'
env.sudo_user = 'www'
env.hosts = ['10.0.0.210']

db_user = 'dbuser'
db_password = 'dbpass'

_TAR_FILE = 'dist-dytt.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/home/www/web/dytt'
...
#     '''
#     dt = _now()
#     f = 'backup-dytt_test-%s.sql' % dt
#     with cd('/tmp'):
#         run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick dytt > %s' % (db_user, db_password, f))
...
def build():
    ...
    Build dist package.
    '''
    includes = ['static', 'templates', 'common', 'config', 'dytt', 'model', 'favicon.ico', '*.py']
...
#     sqls = [
#         'drop database if exists dytt;',
#         'create database dytt;',
#         'grant select, insert, update, delete on dytt.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
#     ]
#     for sql in sqls:
#         local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
#     with lcd(backup_dir):
#         local('tar zxvf %s' % restore_file)
#     local(r'mysql -uroot -p%s dytt < backup/%s' % (p, restore_file[:-7]))
#     with lcd(backup_dir):
